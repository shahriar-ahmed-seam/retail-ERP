/**
 * Customer create / edit form (task 9.2, Phase 9).
 *
 * One form, two modes:
 *
 *   - Create mode (`customer` prop omitted): all fields start blank,
 *     submit calls `customers:upsert` without an `id`.
 *   - Edit mode (`customer` prop provided): fields seeded from the
 *     DTO, submit calls `customers:upsert` with the existing `id`.
 *
 * Validation strategy mirrors the supplier form:
 *   - Cheap client-side checks (required name, length bounds) gate
 *     the submit button. The button stays disabled until the form
 *     is internally consistent.
 *   - Per-field server errors come from the envelope:
 *       * `VALIDATION { field }` — inline next to the offending input.
 *       * `FK_VIOLATION { reason: 'not_found' }` — banner (the row
 *         vanished while the user was editing).
 *       * Anything else — generic banner with code + message.
 *
 * The Customer model has no UNIQUE constraints on user-editable text
 * columns — phone numbers may legitimately repeat across walk-in
 * records — so `UNIQUE_VIOLATION` is not part of this form's surface.
 *
 * Role gating (task 9.2): write access for Admin only. Cashiers
 * (and unauthenticated renderers) see a permission-denied surface
 * instead. Defence-in-depth: the IPC matrix actually authorizes
 * `customers:upsert` for both roles so the POS can create walk-in
 * customer records mid-checkout (Req 7.2), but the dedicated
 * management form is gated to Admin per the task description.
 *
 * On success the form invokes `onClose(saved)` so the parent can
 * snap back to the list view. Standalone consumers (until the route
 * tree lands in task 13.1) get an inline success notice via the
 * `onClose` parent — see also the TODO at the bottom of this file.
 *
 * Validates: Requirements 7.1, 7.2, 8.3.
 */

import {
  useCallback,
  useId,
  useState,
  type FormEvent,
  type ReactElement,
} from 'react';

import { useApi } from '@renderer/lib/api';
import { useAuth } from '@renderer/lib/auth-context';

import type { CustomerDTO, CustomerInput } from '@shared/dto/index';
import type { ErrorEnvelope } from '@shared/result';

// ---------------------------------------------------------------------------
// Props
// ---------------------------------------------------------------------------

export interface CustomerFormPageProps {
  /** When provided the form opens in edit mode, pre-populated from the DTO. */
  readonly customer?: CustomerDTO;
  /**
   * Called when the user successfully submits, or clicks Cancel.
   * Receives the resulting `CustomerDTO` on success; receives
   * `undefined` on cancel.
   */
  readonly onClose: (saved?: CustomerDTO) => void;
}

// ---------------------------------------------------------------------------
// Form state
// ---------------------------------------------------------------------------

/** Bounds mirror `customer.service.ts` so the renderer can give early
 *  feedback before submitting. */
const NAME_MIN = 1;
const NAME_MAX = 100;
const PHONE_MAX = 30;

interface FormState {
  name: string;
  phone: string;
}

function blankState(): FormState {
  return { name: '', phone: '' };
}

function stateFromDTO(c: CustomerDTO): FormState {
  return {
    name: c.name,
    phone: c.phone ?? '',
  };
}

// ---------------------------------------------------------------------------
// Validation
// ---------------------------------------------------------------------------

interface ClientErrors {
  readonly name?: string;
  readonly phone?: string;
}

function validate(state: FormState): ClientErrors {
  const errors: { -readonly [K in keyof ClientErrors]?: string } = {};
  const trimmedName = state.name.trim();
  if (trimmedName.length < NAME_MIN) {
    errors.name = 'Name is required.';
  } else if (trimmedName.length > NAME_MAX) {
    errors.name = `Name must be ${String(NAME_MAX)} characters or fewer.`;
  }
  if (state.phone.trim().length > PHONE_MAX) {
    errors.phone = `Phone must be ${String(PHONE_MAX)} characters or fewer.`;
  }
  return errors;
}

function hasErrors(errors: ClientErrors): boolean {
  return Object.keys(errors).length > 0;
}

/** Build a `CustomerInput` from validated form state. */
function buildInput(state: FormState, existingId: string | undefined): CustomerInput {
  const trimmedPhone = state.phone.trim();
  const base: CustomerInput = {
    name: state.name.trim(),
    phone: trimmedPhone === '' ? null : trimmedPhone,
  };
  if (existingId !== undefined) {
    return { ...base, id: existingId };
  }
  return base;
}

// ---------------------------------------------------------------------------
// Server-error reading
// ---------------------------------------------------------------------------

/**
 * Pull the offending field name out of an error envelope. The customer
 * service writes `{ field: 'name' | 'phone' }` for VALIDATION envelopes.
 */
function readErrorField(error: ErrorEnvelope): string | null {
  const details = error.details;
  if (details === undefined) return null;
  const field = (details as { field?: unknown }).field;
  if (typeof field !== 'string' || field.length === 0) return null;
  return field;
}

// ---------------------------------------------------------------------------
// Top-level component (role gate)
// ---------------------------------------------------------------------------

export function CustomerFormPage(props: CustomerFormPageProps): ReactElement {
  const { session } = useAuth();

  if (session?.role !== 'Admin') {
    return <PermissionDenied />;
  }

  return <CustomerFormPageInner {...props} />;
}

function PermissionDenied(): ReactElement {
  return (
    <main
      role="alert"
      data-testid="customer-form-permission-denied"
      style={{
        fontFamily: 'system-ui, sans-serif',
        padding: '2rem',
        maxWidth: '32rem',
        margin: '4rem auto',
        textAlign: 'center',
        color: '#555',
      }}
    >
      <h1 style={{ marginBottom: '0.5rem' }}>Permission denied</h1>
      <p>
        You do not have permission to manage customer records. Customer
        directory writes are restricted to the Admin role.
      </p>
    </main>
  );
}

// ---------------------------------------------------------------------------
// Form body
// ---------------------------------------------------------------------------

function CustomerFormPageInner({
  customer,
  onClose,
}: CustomerFormPageProps): ReactElement {
  const api = useApi();
  const idPrefix = useId();
  const isEdit = customer !== undefined;

  const [state, setState] = useState<FormState>(() =>
    customer !== undefined ? stateFromDTO(customer) : blankState(),
  );
  const [submitting, setSubmitting] = useState(false);
  const [serverError, setServerError] = useState<ErrorEnvelope | null>(null);

  const setField = useCallback(<K extends keyof FormState>(key: K, value: string): void => {
    setState((prev) => ({ ...prev, [key]: value }));
    setServerError(null);
  }, []);

  const clientErrors = validate(state);
  const canSubmit = !submitting && !hasErrors(clientErrors);

  const handleSubmit = useCallback(
    (event: FormEvent<HTMLFormElement>): void => {
      event.preventDefault();
      if (!canSubmit) return;

      const input = buildInput(state, customer?.id);
      setSubmitting(true);
      setServerError(null);

      void (async () => {
        try {
          const result = await api['customers:upsert'](input);
          if (result.ok) {
            onClose(result.value);
            return;
          }
          setServerError(result.error);
        } finally {
          setSubmitting(false);
        }
      })();
    },
    [api, canSubmit, customer?.id, onClose, state],
  );

  const handleCancel = useCallback((): void => {
    onClose();
  }, [onClose]);

  // Per-field server-error mapping. VALIDATION targets a specific
  // field; everything else falls through to the page-level banner.
  const serverField =
    serverError !== null && serverError.code === 'VALIDATION'
      ? readErrorField(serverError)
      : null;

  const inlineMessage = (key: keyof FormState): string | undefined => {
    const clientMsg = clientErrors[key];
    if (clientMsg !== undefined) return clientMsg;
    if (serverField === key && serverError !== null) {
      return serverError.message;
    }
    return undefined;
  };

  return (
    <main
      style={{
        fontFamily: 'system-ui, sans-serif',
        padding: '1.5rem',
        maxWidth: '40rem',
        margin: '0 auto',
      }}
    >
      <h1 style={{ marginBottom: '1rem' }}>
        {isEdit ? 'Edit customer' : 'New customer'}
      </h1>

      <form onSubmit={handleSubmit} noValidate>
        <Field
          id={`${idPrefix}-name`}
          label="Name"
          required
          value={state.name}
          onChange={(v) => {
            setField('name', v);
          }}
          error={inlineMessage('name')}
        />

        <Field
          id={`${idPrefix}-phone`}
          label="Phone (optional)"
          value={state.phone}
          onChange={(v) => {
            setField('phone', v);
          }}
          error={inlineMessage('phone')}
          inputMode="tel"
        />

        {serverError !== null && serverField === null ? (
          <div
            role="alert"
            data-testid="customer-form-banner"
            style={{
              marginTop: '1rem',
              padding: '0.75rem',
              border: '1px solid #c33',
              color: '#c33',
              background: '#fff5f5',
              borderRadius: 4,
            }}
          >
            <strong>{serverError.code}</strong>
            <div>
              {serverError.code === 'FK_VIOLATION'
                ? 'This customer no longer exists.'
                : serverError.message}
            </div>
          </div>
        ) : null}

        <div style={{ marginTop: '1.5rem', display: 'flex', gap: '0.5rem' }}>
          <button
            type="submit"
            disabled={!canSubmit}
            data-testid="customer-form-submit"
            style={{ padding: '0.625rem 1.25rem' }}
          >
            {submitting ? 'Saving…' : isEdit ? 'Save changes' : 'Create customer'}
          </button>
          <button
            type="button"
            onClick={handleCancel}
            disabled={submitting}
            data-testid="customer-form-cancel"
            style={{ padding: '0.625rem 1.25rem' }}
          >
            Cancel
          </button>
        </div>
      </form>
    </main>
  );
}

// ---------------------------------------------------------------------------
// Field
// ---------------------------------------------------------------------------

interface FieldProps {
  readonly id: string;
  readonly label: string;
  readonly value: string;
  readonly onChange: (next: string) => void;
  readonly required?: boolean;
  readonly error: string | undefined;
  readonly inputMode?: 'text' | 'tel';
}

function Field(props: FieldProps): ReactElement {
  const errorId = props.error !== undefined ? `${props.id}-error` : undefined;

  return (
    <div style={{ marginBottom: '0.875rem' }}>
      <label
        htmlFor={props.id}
        style={{ display: 'block', marginBottom: '0.25rem' }}
      >
        {props.label}
        {props.required === true ? <span aria-hidden="true"> *</span> : null}
      </label>
      <input
        id={props.id}
        type="text"
        aria-invalid={props.error !== undefined}
        {...(errorId !== undefined ? { 'aria-describedby': errorId } : {})}
        required={props.required === true}
        value={props.value}
        onChange={(e) => {
          props.onChange(e.target.value);
        }}
        {...(props.inputMode !== undefined ? { inputMode: props.inputMode } : {})}
        style={{
          width: '100%',
          padding: '0.5rem',
          boxSizing: 'border-box',
          ...(props.error !== undefined ? { borderColor: '#c33' } : {}),
        }}
      />
      {props.error !== undefined ? (
        <div
          id={errorId}
          role="alert"
          data-testid={`${props.id}-error`}
          style={{
            marginTop: '0.25rem',
            color: '#c33',
            fontSize: '0.875rem',
          }}
        >
          {props.error}
        </div>
      ) : null}
    </div>
  );
}
